...
export async function exportData(): Promise<string> {
  const data = {
    ...
    activePlan: await db.activePlan.toArray(),
...
export async function importData(jsonString: string): Promise<void> {
  ...
  await db.transaction('rw', [db.accounts, db.paycheckAllocations, db.weeklyBudgets, db.monthlyBudgets, db.goals, db.netWorthSnapshots, db.settings, db.activePlan], async () => {
    ...
    if (data.activePlan) await db.activePlan.bulkAdd(stripId(data.activePlan));
  });
}
